...
- 🔒 **"Ödemeyi Simüle Et" butonunun görünürlük koşulu = sağlayıcı sahte mi, ortam ne ise o.** Kullanıcı talebi üzerine buton canlıda da açıldı. Bu yüzden koşul artık `IsDevelopment()` **değil**, yalnızca `IPaymentProvider is FakePaymentProvider`'dır (bkz. `PaymentsDevController`, `web/app/(site)/odeme/[invoiceId]/page.tsx`). Gerekçe: doğru güvenlik sınırı ortam adı değil, **paranın gerçek olup olmamasıdır** — `Payment:Mode` `Sandbox`/`Live` yapıldığı anda sağlayıcı değişir, uç 404'e döner ve buton kendiliğinden kaybolur (fail-closed; kapatılmayı bekleyen bir bayrak yoktur). Görünürlük kararı frontend'de `NODE_ENV` ile derleme zamanında değil, `GET /api/dev/payments/availability` ile **çalışma zamanında sunucudan** alınır; böylece "buton var ama uç yok" tutarsızlığı yapısal olarak imkânsızdır.
  - Uç `[Authorize]`'dır ve yalnızca **çağıranın kendi** invoice'ını ödenmiş sayar (başkasınınki 404). Bu iki koruma buton yalnızca Development'tayken yoktu; canlıya açılırken eklendi — aksi halde kimliksiz bir istekle başkasının invoice'ı kapatılabilirdi.
  - ⚠️ Bu buton `Fake` modda **karşılığı olmayan bakiye üretir**. `Fake` modda çekim de sahte sağlayıcıdan geçtiği için zincire hiçbir şey çıkmaz, yani gerçek para riski yoktur. Canlıyı gerçek paraya geçirirken yapılacak tek şey `Payment:Mode`'u değiştirmektir; kodda ayrıca kapatılacak bir şey yoktur.
